...
export const useCourse = (courseId) => {
    const {
        ...
        courseId ? `/api/course/courseTermInfo/${ courseId }` : null, 
        jsonFetcher, {
...
export const usePostContent = (postId, authorId, page, loadBefore) => {
    ...
    const {
        ...
        doFetch ? 
        `/api/course/postsInfo/${ postId }/content/postAndTopLevelComments/${ authorId }/${ page }/${ loadBefore }`
        : null,
        jsonFetcher,
        {
            revalidateIfStale: false,
...
export const useMoreReplies = (postId, ancestorId, page, threadOffset) => {
    ...
    const {
        ...
        doFetch ? 
        `/api/course/postsInfo/${ postId }/content/lowerLevelComments/${ ancestorId }/${ page }/${ threadOffset }`
        : null,
        jsonFetcher,
        {
            revalidateIfStale: false,
...
export const useNotifications = (afterTime) => {
    const { data, error } = useSWR(
        `/api/notifications/${ afterTime }`,
        jsonFetcher, {
            revalidateIfStale: false,
            revalidateOnFocus: false,
            revalidateOnReconnect: false,
    ...
    const loading = error === undefined && data === undefined
    if (!data) return { notifications: [], error, loading }

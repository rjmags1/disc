...
async (client, mentions, genId, genIsPost) => {
    ...
    const genMentionNotifQueryTextTokens = [`INSERT INTO notification 
        (${ genIsPost ? 'gen_post' : 'gen_comment'}, is_mention_noti, person) 
        VALUES `]
    const genMentionNotifParams = [genId, true]
    ...
    await clientQuery(
        client, genMentionNotifQueryTextTokens.join(''), genMentionNotifParams)
})

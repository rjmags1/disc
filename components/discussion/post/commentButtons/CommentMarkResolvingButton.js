...
const CommentMarkResolvingButton = React.memo(function(props) {
    const { commentInfo, postId, isResolving, setPostResolved, setCommentResolving } = props
    const { 
    ...
    const handleClick = async () => {
        try {
            ...
            setPostResolved(newStatus) // tell post to rerender w/ or w/o checkmark in post header
            setCommentResolving(newStatus) // tell comment to rerender as resolving comment
            ...
            // rerender associated listings based on resolution status (ie, remove checkmark or add one)

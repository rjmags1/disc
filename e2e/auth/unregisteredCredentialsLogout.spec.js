...
test.describe('valid unregistered inputs rejected', async () => {
    ...
    const [registered, unregistered] = [TESTUSER_REGISTERED, TESTUSER_UNREGISTERED]

    test('unregistered org rejected -- full login', async ({ page }) => {
        await page.goto('/login')
        await page.locator('[id=organization-input]').fill(unregistered.organization)
        ...
        const invalidMessage = await page.waitForSelector('text=/invalid/gi', waitOptions)
        expect(invalidMessage).not.toBeNull()
    })

    test('unregistered email rejected -- full login', async ({ page }) => {
        await page.goto('/login')
        await page.locator('[id=organization-input]').fill(registered.organization)
    ...
    test('existing user but incorrect password rejected -- full login', async ({ page }) => {
        await page.goto('/login')
        await page.locator('[id=organization-input]').fill(registered.organization)
        await page.locator('[id=email-input]').fill(registered.email)
